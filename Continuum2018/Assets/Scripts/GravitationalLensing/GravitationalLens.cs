...
public class GravitationalLens: MonoBehaviour 
{
	...
	public float   ratio = 0.5625f;  	// The ratio of the height to the length of the screen to display properly shader
	...
	void OnRenderImage (RenderTexture source, RenderTexture destination)
	{
		if (shader && material) 
		{
			...
			if (useLocalPos == true) 
			{
				...
				Vector2 pos = new Vector2 
					(
						cam.WorldToScreenPoint (BH.transform.localPosition).x / cam.pixelWidth,
						cam.WorldToScreenPoint (BH.transform.localPosition).y / cam.pixelHeight
					);
