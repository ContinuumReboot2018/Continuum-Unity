...
namespace PowerTools
{
	...
	public class SelectorLog : EditorWindow
	{
		#region Definitions
		...
		static readonly int MAX_ITEMS = 15;
		...
		static void ShowWindow()
		{
			...
			EditorWindow.GetWindow<SelectorLog>("Selection Log");
		}
		...
		void OnGUI ()
		{
			...
			Dictionary<System.Type, List<SelectedObject>> ObjectsByType = new Dictionary<System.Type, List<SelectedObject>> ();
			...
			{
				if (!ObjectsByType.ContainsKey (m_selectedObjects [i].m_object.GetType ()))
				{
					ObjectsByType.Add (m_selectedObjects [i].m_object.GetType (), new List<SelectedObject>());
				}

				ObjectsByType [m_selectedObjects [i].m_object.GetType ()].Add (m_selectedObjects [i]);
			}

			foreach (var obt in ObjectsByType)
			{
				string typeName = obt.Key.ToString ();

				if (typeName.IndexOf ("UnityEditor.")>0)
					typeName = typeName.Substring (typeName.IndexOf ("UnityEditor."));
				if (typeName.IndexOf ("UnityEngine.")>0)
					typeName = typeName.Substring (typeName.IndexOf ("UnityEngine."));

				for (int i = obt.Value.Count - 1; i >= 0; --i)
				{
					if (obt.Value [i].m_locked == false)
					{
						if (processingLocked)
						{
					...
					LayoutItem (i, obt.Value [i], obt.Key);
...
*/
		...
		void LayoutItem(int i, SelectedObject obj, System.Type sysType)
		{
			...
			if (  obj != null && obj.m_object != null)
			{
				...
				*/
				...
				GUILayout.Label (EditorGUIUtility.ObjectContent(null, sysType).image, GUILayout.Height(20f), GUILayout.Width(20f));
